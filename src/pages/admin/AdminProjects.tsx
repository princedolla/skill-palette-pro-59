...
const AdminProjects = () => {
  ...
  const emptyProject = { title: "", description: "", category: "Web Dev", tech_stack: [], image_url: "", live_url: "", github_url: "", featured: false, published: true };
      ...
      {showForm && editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <form onSubmit={handleSave} className="w-full max-w-lg bg-card border border-border rounded-xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <label className="text-xs font-mono text-muted-foreground mb-1 block">Tech Stack (comma-separated)</label>
              <input type="text" value={(editing.tech_stack || []).join(", ")} onChange={(e) => setEditing({ ...editing, tech_stack: e.target.value.split(",").map((s: string) => s.trim()).filter(Boolean) })} className="w-full px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-primary transition-colors" />
            </div>
            <div className="flex items-center gap-4">
              ...
              <label className="flex items-center gap-2 text-sm text-foreground">
                <input type="checkbox" checked={editing.featured} onChange={(e) => setEditing({ ...editing, featured: e.target.checked })} className="accent-primary" /> Featured
              </label>
            </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {projects?.map((project) => (
                <tr key={project.id} className="border-t border-border hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3 text-foreground font-medium">{project.title}</td>
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{project.category}</td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className={`px-2 py-0.5 rounded text-xs font-mono ${project.published ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                      {project.published ? "Published" : "Draft"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => openForm(project)} className="p-1.5 rounded text-muted-foreground hover:text-primary transition-colors"><Pencil size={14} /></button>
                    <button onClick={() => deleteMutation.mutate(project.id)} className="p-1.5 rounded text-muted-foreground hover:text-destructive transition-colors ml-1"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
              {(!projects || projects.length === 0) && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">No projects yet. Add your first one!</td></tr>
              )}
            </tbody>
          </table>
        </div>
